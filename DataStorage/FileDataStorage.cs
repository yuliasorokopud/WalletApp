...
namespace DataStorage
{
    public class FileDataStorage<TObject> where TObject : class, IStorable
    {
        ...
        {
            string stringObj = JsonSerializer.Serialize(obj);

            using StreamWriter sw = new StreamWriter(Path.Combine(BaseFolder, obj.Guid.ToString("N")), false);
            await sw.WriteAsync(stringObj);       
        }

        public void DeleteObj(TObject obj)
        {
            File.Delete(Path.Combine(BaseFolder, obj.Guid.ToString("N")));
        }

        public TObject Get(Guid guid)
        {
